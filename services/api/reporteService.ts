...
class ReporteService {
  ...
  async exportarExcel(
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/export/excel${params.toString() ? '?' + params.toString() : ''}`;
  ...
  async exportarCSV(
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/export/csv${params.toString() ? '?' + params.toString() : ''}`;
